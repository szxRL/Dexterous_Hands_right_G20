...
class PressureDiagram(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
    ...
    def subscribe_waveform(self, topic):
        if self.wave_sub:
            self.wave_sub.unregister()
        self.wave_sub = rospy.Subscriber(topic, String, self.wave_callback, queue_size=100)
        rospy.loginfo(f"WAVEFORM: {topic}")

    def subscribe_matrix(self, topic):
        if self.matrix_sub:
            self.matrix_sub.unregister()
        self.matrix_sub = rospy.Subscriber(topic, String, self.matrix_callback, queue_size=100)
        rospy.loginfo(f"MATRIX: {topic}")
        ...
        try:
            ...
            for finger in self.fingers_matrix:
                ...
                if arr.ndim == 2:
                    ...
                    rospy.loginfo_throttle(3.0, f"{finger}: {rows}×{cols}, max={arr.max():.1f}")
                    
        except Exception as e:
            rospy.logerr_throttle(5.0, f"Matrix error: {str(e)}")
...
if __name__ == '__main__':
    rospy.init_node('pressure_diagram', anonymous=True)
