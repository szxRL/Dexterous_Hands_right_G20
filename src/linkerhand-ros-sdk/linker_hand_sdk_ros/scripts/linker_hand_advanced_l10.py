...
class LinkerHandAdvancedL10:
    def __init__(self,hand_type="right", hand_joint="L10", is_touch="true", can="can0", moudbus="None"):
    ...
    def joint_state_msg(self, pose,vel=[]):
        ...
        joint_state.name = ["thumb_cmc_pitch", "thumb_cmc_roll", "index_mcp_pitch", "middle_mcp_pitch", "ring_mcp_pitch", "pinky_mcp_pitch", "index_mcp_roll", "ring_mcp_roll", "pinky_mcp_roll", "thumb_cmc_yaw"]
    ...
    def get_state(self):
        while True:
            ...
            self.last_hand_state['vel'] = [0.0] * 10
                #time.sleep(0.003)
            """矩阵式压力传感器"""
            self.matrix_dic["thumb_matrix"] = self.api.get_thumb_matrix_touch(sleep_time=0.004).tolist()
            self.matrix_dic["index_matrix"] = self.api.get_index_matrix_touch(sleep_time=0.004).tolist()
            self.matrix_dic["middle_matrix"] = self.api.get_middle_matrix_touch(sleep_time=0.004).tolist()
...
if __name__ == '__main__':
    ...
    '''
    rospy.init_node('linker_hand_advanced_l10', anonymous=True)
    ...
    linker_hand = LinkerHandAdvancedL10(hand_type=args.hand_type, is_touch=args.is_touch, can=args.can)
