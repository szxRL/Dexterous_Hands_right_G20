...
class ROS1NodeManager(QObject):
    ...
    def init_ros(self):
        try:
            ...
            self.status_updated.emit("info", f"ROS1节点初始化成功: {self.hand_type} {self.hand_joint}")
        except Exception as e:
            self.status_updated.emit("error", f"ROS初始化失败: {str(e)}")
            raise

    # ---------- 下面三个发布函数改成 rospy.Time / rospy.Publisher ----------
    def publish_joint_state(self, positions: List[int]):
        if not self.pub:
            self.status_updated.emit("error", "ROS发布者未初始化")
            return
        try:
            ...
            if self.is_arc:
                pose = self.calc_arc(positions)   # 把你原来的弧度转换包一层
